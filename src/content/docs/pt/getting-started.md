...
description: Instale o Windup, inicialize um projeto e vá de uma instrução vaga a um teste determinístico com replay em cinco comandos.
---
...
- **Node ≥ 20.**
- Uma **chave de API** para o LLM planejador em `.env.local` ou `.env` (`.env.local` prevalece — use-o quando o seu `.env` estiver versionado): `GOOGLE_GENERATIVE_AI_API_KEY` para Google (padrão) ou `OPENAI_API_KEY` para OpenAI.
...
npx windup init
#    → windup.config.ts + e2e/scenarios/ + .windup/ (cache & map versionados; state/runs/reports no gitignore)
