...
export default defineConfig({
  ...
  // Barreiras de saúde em runtime: falha um cenário diante de um erro de console / 5xx durante o run.
...
- **`failOn`** transforma sinais de saúde em runtime em falhas. `consoleErrors: true` falha um cenário que logou um erro de console ou lançou uma exceção não capturada; `http5xx: true` falha um cuja página recebeu um 5xx. `ignore` é uma lista de substrings de URL/texto para silenciar ruído conhecido (analytics, um 500 de terceiros que você não controla). Requisições respondidas pelo `config.network` são sempre excluídas — um stub deliberado não é uma falha real. As flags `--fail-on-console` / `--fail-on-5xx` as forçam para um run; de qualquer forma os sinais são registrados e mostrados nos relatórios.
