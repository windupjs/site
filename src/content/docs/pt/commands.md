...
| `--fail-on-console` | Falha um cenário em um erro de **JS** durante o run — uma exceção não capturada, um `console.error` ou uma violação de CSP (registrado de qualquer forma; `config.failOn.ignore` casa por mensagem ou URL). |
| `--fail-on-resource` | Falha um cenário se um **sub-recurso** (img/font/script/xhr) falhou ao carregar com um 4xx durante o run — uma barreira separada de `--fail-on-console` para que imagens quebradas não afoguem os erros de JS. |
