...
- **Barreiras de saúde em runtime — `--fail-on-console` / `--fail-on-5xx`.** Um cenário pode "passar" enquanto a página logou um erro de JS ou recebeu um 5xx silencioso. O Windup observa ambos passivamente durante o run e **os registra sempre** (mostrados nos relatórios de console/HTML/JSON, informativos como `--a11y`); estas flags (ou `config.failOn`) os transformam em **falha**. Stubs deliberados do `config.network` (ex.: um `{ status: 500 }`) e URLs de `config.failOn.ignore` são excluídos, então testar um estado de erro nunca dispara a barreira.
