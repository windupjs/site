...
- **Uma verificação que não pode falhar é rejeitada.** Se a checagem final do plano apenas afirma que um *landmark* está visível (`body`, `html`, `main`, `div`, `section`, `#root`, …), ela passa desde que a página carregue — uma execução verde que não prova nada. O Windup **rejeita** um plano assim na validação e re-planeja, instruindo o modelo a asserir algo específico. Por isso, **diga na tarefa o que deve ser verificado** ("…e verifique que o texto 'Popular parking' aparece", "…e verifique que 3 linhas são listadas") — é isso que vira uma asserção de verdade. Asserções de conteúdo valem em qualquer seletor: `text_contains: { selector: "main", text: "Popular parking" }` passa, porque o que ela afirma é o texto. Planos já em cache seguem sendo reproduzidos sem alteração; rode `windup explain <id>` para identificar uma verificação fraca que você já tem (ele imprime `⚠ weak verification: …`).
