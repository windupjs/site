...
- **Fallar rápido — `--all --bail`.** Deja de iniciar nuevos escenarios tras la **primera falla** — feedback rápido en un check de PR en vez de esperar toda la suite. Completa el trío de barreras con `--retries`/`--max-wall`; funciona en secuencial y con `--concurrency`.
- **Cuarentena de un escenario flaky (`scenario.quarantine: true`).** Un escenario en cuarentena igual corre y reporta, pero su falla **no hace fallar el build** — un flake terco deja de bloquear el CI mientras lo arreglas, en vez de borrar el test o dejar que enrojezca cada run. Se muestra de forma llamativa (una línea `🔶`, una insignia `QUARANTINED` en el reporte); `windup trends <id>` muestra si se estabilizó.
...
- **Precalentamiento del navegador (activado por defecto; `--no-prewarm`).** En un `run --all` secuencial, mientras un escenario corre Windup pre-crea el contexto+página **fresco** que necesita el *siguiente* — su lanzamiento de ~200 ms ocurre fuera del camino crítico en vez de como espera muerta (el segmento `setup` de un escenario cae a ~0). El isolation no cambia: cada escenario sigue teniendo su propio contexto limpio. Solo los runs secuenciales precalientan — `--concurrency > 1` ya solapa los lanzamientos entre workers.
- **Barreras de salud en runtime — `--fail-on-console` / `--fail-on-5xx`.** Un escenario puede "pasar" mientras la página registró un error de JS o recibió un 5xx silencioso. Windup observa ambos de forma pasiva durante el run y **los registra siempre** (mostrados en los reportes de consola/HTML/JSON, informativos como `--a11y`); estas banderas (o `config.failOn`) los convierten en **falla**. Los stubs deliberados de `config.network` (p. ej. un `{ status: 500 }`) y las URLs de `config.failOn.ignore` se excluyen, así que probar un estado de error nunca dispara la barrera.
