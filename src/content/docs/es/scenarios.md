...
- **Una verificación que no puede fallar es rechazada.** Si el chequeo final del plan solo aserta que un *landmark* es visible (`body`, `html`, `main`, `div`, `section`, `#root`, …), pasa con que la página cargue — una ejecución en verde que no prueba nada. Windup **rechaza** ese plan en la validación y vuelve a planificar, indicándole al modelo que aserte algo específico en su lugar. Así que **nombra en la tarea aquello que hay que verificar** ("…y verifica que aparece el texto 'Popular parking'", "…y verifica que se listan 3 filas") — eso es lo que se convierte en una aserción real. Las aserciones de contenido valen sobre cualquier selector: `text_contains: { selector: "main", text: "Popular parking" }` pasa, porque lo que aserta es el texto. Los planes ya cacheados siguen reproduciéndose intactos; ejecuta `windup explain <id>` para detectar una verificación débil que ya tengas (imprime `⚠ weak verification: …`).
