...
export default defineConfig({
  ...
  // Barreras de salud en runtime: falla un escenario ante un error de JS / 4xx de recurso / 5xx visto durante el run.
...
Cada sección de abajo es opcional — una config recién creada por `windup init` solo define `baseUrl`, `llm` y `scenarios`.

## Manifiesto y credenciales
...
## Determinismo y stub de peticiones
...
## Barreras de salud en runtime

- **`failOn`** convierte señales de salud en runtime en fallas. `consoleErrors: true` falla ante un error de **JS** — una excepción no capturada, un `console.error`, una violación de CSP; `resourceErrors: true` falla ante un **sub-recurso** que no logró cargar (un 4xx de img/font/script/xhr — la clase ruidosa, mantenida como barrera separada para que la salud de JS no se ahogue en imágenes rotas); `http5xx: true` falla ante un 5xx. `ignore` es una lista de substrings que silencian ruido conocido (analytics, un `d=404` de Gravatar, un 500 de terceros que no controlas) — cotejada contra **tanto el mensaje como la URL de origen**, así un error de recurso cuyo texto de consola no lleva URL igual se puede silenciar por su host. Las peticiones respondidas por `config.network` siempre se excluyen — un stub deliberado no es una falla real, ya sea el stub global o **por escenario** (el error que produce, respuesta o consola, se coteja por URL contra las reglas efectivas del run). Las banderas de la CLI `--fail-on-console` / `--fail-on-resource` / `--fail-on-5xx` las fuerzan para un solo run; en cualquier caso las señales se registran (cada error de consola con su `url`, su `kind` `js`/`resource` y — para un error de recurso — el `status` HTTP) y se muestran en los reportes. Ajustable también **por escenario** (`Scenario.failOn`, fusionado sobre este global — los booleanos ganan, `ignore` se concatena) para abrir una excepción de un solo escenario sin un `ignore` de toda la suite.
