...
- **不可能失败的验证会被拒绝。** 如果计划的最终检查只断言某个*地标元素*可见（`body`、`html`、`main`、`div`、`section`、`#root`……），那么只要页面加载出来它就会通过 —— 一次什么都没证明的绿色运行。Windup 会在校验阶段**拒绝**这样的计划并重新规划，要求模型改为断言某个具体的东西。所以请**在任务中点名要验证什么**（"……并验证出现文本 'Popular parking'""……并验证列出了 3 行"）—— 那才是会变成真正断言的部分。内容断言用在任何选择器上都没问题：`text_contains: { selector: "main", text: "Popular parking" }` 会通过，因为它断言的是文本。已缓存的计划照常回放、不受影响；运行 `windup explain <id>` 就能发现你现有的薄弱验证（它会打印 `⚠ weak verification: …`）。
