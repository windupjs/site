...
- **运行时健康门禁 —— `--fail-on-console` / `--fail-on-5xx`。** 一个场景可能在页面记录了 JS 错误或收到静默 5xx 的情况下仍"通过"。Windup 在运行期间被动观察这两者并**始终记录**（显示在 console/HTML/JSON 报告中，像 `--a11y` 一样是信息性的）；这些标志（或 `config.failOn`）把它们变成**失败**。`config.network` 刻意设置的桩（例如 `{ status: 500 }`）以及 `config.failOn.ignore` 中的 URL 会被排除，因此测试一个错误状态永远不会触发门禁。
