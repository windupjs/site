...
- **提交计划缓存，让 CI 无需 LLM（这正是关键所在）。** 回放之所以是 `$0`，是因为它使用*缓存的计划*而非规划器 —— 因此对于 CI（以及任何没有你规划器的 CLI/密钥的机器），计划缓存必须**被提交**。`windup init` 会替你配置好这一点：它把 `.windup/cache/`（计划）和 `.windup/map/`（站点地图）纳入版本控制，而只把 `.windup/state/`（认证 cookie）、`.windup/runs/` 和 `.windup/reports/` 加入 gitignore。在本地为整个套件规划一次，`git add .windup`，之后 CI 就能以 $0 回放每个场景，**无需 LLM，也无需 Claude CLI**。计划是可移植且不含密钥的（选择器 + `value_ref`，从不含已解析的值）。当一个**新**场景（或一次真实的 UI 变更）导致 `cache=miss` 时，只有那一个需要规划器 —— 在本地重新生成并提交，或者给 CI 一个**非交互式**规划器（带 API 密钥的 `--llm google`/`--llm openai`，或通过 `WINDUP_CLAUDE_CODE_URL` 使用 `claude-code-openai-wrapper`）。*（升级一个根 `.gitignore` 里有笼统 `.windup/` 的老项目？把它收窄为 `.windup/state/`、`.windup/runs/`、`.windup/reports/`，并提交 `.windup/cache/`。）*
...
- **隔离一个 flaky 场景（`scenario.quarantine: true`）。** 被隔离的场景仍会运行并报告，但它的失败**不会让构建失败** —— 一个顽固的 flake 在你修复它期间不再阻塞 CI，而无需删除测试或任由它每次都变红。它会被醒目地呈现（一行 `🔶`、报告中的 `QUARANTINED` 徽章）；`windup trends <id>` 显示它是否已经稳定。
...
- **浏览器预热（默认开启；`--no-prewarm`）。** 在顺序的 `run --all` 中，当一个场景运行时，Windup 会预先创建*下一个*场景所需的**全新** context+page —— 它约 200 ms 的启动发生在关键路径之外，而不是变成空等（一个场景的 `setup` 段降到约 0）。隔离不变：每个场景仍获得自己干净的 context。只有顺序运行会预热 —— `--concurrency > 1` 已经在各 worker 间重叠了启动。
- **运行时健康门禁 —— `--fail-on-console` / `--fail-on-resource` / `--fail-on-5xx`。** 一个场景可能在页面抛出了 JS 错误、加载资源失败或收到静默 5xx 的情况下仍"通过"。Windup 被动观察这一切并**始终记录**（显示在 console/HTML/JSON 报告中，像 `--a11y` 一样是信息性的）；这些标志（或 `config.failOn`）把它们变成**失败**。两类 console 噪声被**分开**处理：`--fail-on-console` 对 **JS** 错误（未捕获的异常、`console.error`、CSP 违规）设门禁，而 `--fail-on-resource` 对**子资源 4xx 加载**（损坏的图片/字体/xhr）设门禁——因此你可以在不被损坏图片淹没的情况下对 JS 健康设门禁。每条记录的 console 错误都带有其**来源 URL** 以及 `js`/`resource` `kind`，`config.failOn.ignore` 则**按消息或 URL** 静默噪声。`config.network` 刻意设置的桩（例如 `{ status: 500 }`）会被排除，因此测试一个错误状态永远不会触发门禁。
- **设备视口 —— `--device "<preset>"` / `config.device`。** 在一个 Playwright 设备预设（`"iPhone 14"`、`"Pixel 7"`、`"iPad Pro 11"`）下运行套件 —— 视口、user-agent、移动/触摸。缓存计划按设备分键，因此移动端 CI job 与桌面端保持独立轨迹。移动端模拟需要 chromium。
- **性能预算 —— `--web-vitals` / `config.budgets`。** 捕获最终页面的 TTFB/FCP/LCP/DCL/load/CLS（像 `--a11y` 一样报告）；设置 `config.budgets`（`{ lcp_ms, cls, load_ms, … }`），超标即让构建失败（kind `budget`）。时序是有噪声的 —— 预算要留出余量，用于抓回归而非抖动。
