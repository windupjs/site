...
| `--fail-on-console` | Fail a scenario on a **JS** error during the run — an uncaught exception, a `console.error`, or a CSP violation (recorded either way; `config.failOn.ignore` matches by message or URL). |
| `--fail-on-resource` | Fail a scenario if a **sub-resource** (img/font/script/xhr) failed to load with a 4xx during the run — a separate gate from `--fail-on-console` so broken images don't drown JS errors. |
