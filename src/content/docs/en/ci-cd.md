...
- **Quarantine a flaky scenario (`scenario.quarantine: true`).** A quarantined scenario still runs and reports, but its failure **does not fail the build** — one stubborn flake stops blocking CI while you fix it, instead of deleting the test or letting it redden every run. Surfaced loudly (a `🔶` line, a `QUARANTINED` report badge); `windup trends <id>` shows whether it has stabilized.
...
- **Browser prewarming (on by default; `--no-prewarm`).** In a sequential `run --all`, while a scenario runs Windup pre-creates the **fresh** context+page the *next* one needs — its ~200 ms launch happens off the critical path instead of as dead wait (a scenario's `setup` segment drops to ~0). Isolation is unchanged: every scenario still gets its own clean context. Only sequential runs prewarm — `--concurrency > 1` already overlaps launches across workers.
- **Runtime health gates — `--fail-on-console` / `--fail-on-resource` / `--fail-on-5xx`.** A scenario can "pass" while the page threw a JS error, failed to load a resource, or got a silent 5xx. Windup observes all of it passively and **records it always** (shown in the console/HTML/JSON reports, informational like `--a11y`); these flags (or `config.failOn`) turn them into a **failure**. The two console-noise classes are **separated**: `--fail-on-console` gates **JS** errors (uncaught exceptions, `console.error`, CSP violations) while `--fail-on-resource` gates **sub-resource 4xx loads** (a broken image/font/xhr) — so you can gate JS health without drowning in broken images. Each recorded console error carries its **originating URL** and a `js`/`resource` `kind`, and `config.failOn.ignore` silences noise **by message or URL**. Deliberate `config.network` stubs (e.g. a `{ status: 500 }`) are excluded, so testing an error state never trips the gate.
- **Device viewports — `--device "<preset>"` / `config.device`.** Run the suite at a Playwright device preset (`"iPhone 14"`, `"Pixel 7"`, `"iPad Pro 11"`) — viewport, user-agent, mobile/touch. Cached plans are keyed per device, so a mobile CI job and a desktop one keep separate trajectories. Mobile emulation needs chromium.
- **Performance budgets — `--web-vitals` / `config.budgets`.** Capture the final page's TTFB/FCP/LCP/DCL/load/CLS (reported like `--a11y`); set `config.budgets` (`{ lcp_ms, cls, load_ms, … }`) and a breach fails the build (kind `budget`). Timing is noisy — set budgets with headroom so they catch regressions, not jitter.
