...
- **Quote the text you want verified — Windup asserts it for you.** When the task names a literal in quotes ("…and verify the text **'Popular parking'** appears") and the plan's final check turns out to be weak, Windup rewrites it into a `text_contains` on that exact text — **deterministically, with no extra LLM call**, and only after confirming the live page really contains it. This is the reliable way to get a strong assertion: it does not depend on the model choosing well.
- **A verification that cannot fail is rejected.** A final check is worthless when it passes regardless of the feature: a bare *landmark* (`body`, `html`, `main`, `div`, `#root`) or **any bare selector that matches more than one element on the live page** (`h2` on a page with five headings — deleting the whole section still leaves four). Windup checks both — the landmark list offline, the match count against the real page at plan time — and **rejects** such a plan, re-planning with an error that says how many elements matched and what to assert instead. Content assertions are fine on any selector: `text_contains: { selector: "main", text: "Popular parking" }` passes, because what it asserts is the text. Already-cached plans keep replaying untouched; run `windup explain <id>` to spot a weak verification you already have (it prints `⚠ weak verification: …`).
...
- **Per-scenario determinism (`network` / `clock`).** The `network` request stubs and `clock` freeze that `windup.config.ts` exposes globally can also live on a **single scenario** — scoped to just that run and **merged over the global config, scenario winning**. That's how you test one error state without collateral: `{ "scenario_id": "erro-lista-500", "network": [{ "url": "v1/passports", "status": 500 }] }` forces a 500 on that endpoint **only here** (global stubs still apply; the normal listing scenario is untouched). `clock` merges field-wise (scenario `now`/`timezone` override global). Applied at context launch, never cached — the plan is planned against the stubbed page, so an error-UI assertion is stable. (A scenario with its own `network`/`clock` skips browser prewarming — the prewarmed context only carries the global config.) The error a stub deliberately produces is **excluded from the `failOn` gate** — a stubbed `500` won't trip `--fail-on-resource`/`--fail-on-5xx`.
- **Per-scenario `failOn`.** Open a runtime-health exception for **one** scenario instead of blinding the whole suite: `{ "scenario_id": "…", "failOn": { "resourceErrors": false } }`, or a scenario-only `"ignore"`. It merges over the global `config.failOn` — the boolean gates take the scenario's value when set, and `ignore` lists **concatenate** (global noise + this scenario's). So a URL you only need to silence for one listing stops being an `ignore` entry every other scenario also carries.
...
- **Guided self-heal.** A re-plan tells the planner the exact selector that failed ("don't reuse it"), re-emphasizes your hints, and — with `--suggest` — feeds the same expert diagnosis you'd read back into the re-plan, so it corrects instead of re-proposing a refuted selector. If a scenario keeps re-planning without stabilizing, Windup warns that the app likely lacks a stable selector (an accessibility gap) or has a race, instead of churning silently. **A caught regression always outranks the self-heal that follows it:** if the re-plan *also* fails (a truncated reply, a bad key), the run still reports the **postcondition failure** as the headline — with its expected/actual — and demotes the re-plan's own trouble to a `note:` line. The tool's hiccup never speaks louder than the bug it just found.
