...
- Initialize a project: \`npx windup init\`. Index the app's routes/elements from source: \`npx windup scan\`. Find coverage gaps (indexed routes with no scenario): \`npx windup coverage\`. Preflight before CI: \`npx windup doctor\`.
...
- Run: \`npx windup run <scenario_id>\` (first run plans, later runs replay at $0). Whole suite for CI: \`npx windup run --all --reporter junit\` (non-zero exit on failure). Incremental CI: \`--changed\` (vs HEAD) or \`--since <ref>\` runs only scenarios a change affects, with a safe full-suite fallback. Resilient CI: \`--retries N\` re-runs a transient flake (network/verification/setup/dependency, never a forbidden block) and flags it \`flaky\` if it only passes on a retry; \`--max-wall <seconds>\` caps suite wall-clock and exits non-zero if exceeded. Runtime values (OTP/magic-link) come from author-declared \`config.resolve\` sources (cmd/http/fn), referenced via \`value_ref\`/\`url_ref\` — the value is fetched with polling and never cached.
- A scenario is JSON: \`{ "scenario_id", "start_url"?, "task", "hints"?, "depends_on"?, "setup"?, "teardown"?, "like"?, "seed"?, "requires"?, "tags"?, "on_dialog"?, "atomic_steps"? }\`. \`requires\` is a list of human-readable DATA preconditions (e.g. "1 active attraction"), shown in the report; \`tags\` selects a subset with \`run --all --tag\`. \`on_dialog: "accept"|"dismiss"\` installs a persistent handler that answers every native confirm/alert for the whole run; \`atomic_steps: true\` forces the planner to emit one interaction per action (no merged reveal-then-act). When a plan selector misses at replay, Windup retries the target by its accessible name (label/placeholder/role, exactly-one-visible-match) and reports an a11y gap when nothing matches. \`seed: { localStorage?, sessionStorage?, origin? }\` injects browser storage before the plan runs (a cart, a POS device) — deterministic, no server call, great for CI-safe client-side tests. End the \`task\` with what to verify — it becomes the final postcondition. Never put secrets in tasks; reference named accounts (\`windup secret set <account>\`), plans use \`value_ref: "ENV:VAR"\` resolved at runtime. Prefer idempotent scenarios; for a non-idempotent write (e.g. a CREATE with a unique key), use \`setup\`/\`teardown\` shell hooks that run outside the cached plan (fixtures / cleanup). \`like: { scenario, set? }\` reuses another scenario's proven plan (same flow, different route/values) with no LLM call — still executed and verified, falling back to LLM planning on any mismatch.
