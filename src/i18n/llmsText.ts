...
- A scenario is JSON: \`{ "scenario_id", "start_url"?, "task", "hints"?, "depends_on"?, "setup"?, "teardown"? }\`. End the \`task\` with what to verify — it becomes the final postcondition. Never put secrets in tasks; reference named accounts (\`windup secret set <account>\`), plans use \`value_ref: "ENV:VAR"\` resolved at runtime. Prefer idempotent scenarios; for a non-idempotent write (e.g. a CREATE with a unique key), use \`setup\`/\`teardown\` shell hooks that run outside the cached plan (fixtures / cleanup).
